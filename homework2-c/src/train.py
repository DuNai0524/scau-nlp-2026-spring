"""Training helpers for Homework 2-c (TextCNN)."""
...
def normalize_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for key in [
        "num_filters", "max_length", "batch_size", "num_train_epochs",
        "early_stop_patience", "seed",
    ]:
        merged[key] = int(merged[key])
    for key in ["dropout", "learning_rate", "weight_decay", "label_smoothing", "freeze_embeddings"]:
        merged[key] = float(merged[key]) if key != "freeze_embeddings" else bool(merged[key])
    if isinstance(merged["filter_sizes"], str):
        merged["filter_sizes"] = [int(x) for x in merged["filter_sizes"].split(",")]
    return merged
...
def train_and_save(
    ...
    # Optimizer — only train non-embedding params
    ...
    best_dev_acc = 0.0
    patience_counter = 0
    model_path = os.path.join(results_dir, "model", "textcnn.pt")
    ...
    # Save artifacts
    config["filter_sizes"] = [int(x) for x in config["filter_sizes"]]
